...
require File.join(File.expand_path('../', __FILE__), 'tomcat7.rb')

class Tomcat7Plugin < StagingPlugin
  ...
  def configure_catalina_opts
    new_memory = (application_memory * 0.3).ceil
    # We want to set this to what the user requests, *not* set a minum bar
	"-server -Xms#{application_memory}m -Xmx#{application_memory}m -Xmn#{new_memory}m -XX:MaxNewSize=#{new_memory}m -XX:PermSize=128m -XX:MaxPermSize=128m -Xloggc:../logs/gc.log -XX:+PrintGCTimeStamps -XX:+PrintGCDetails -XX:+PrintClassHistogram -XX:+HeapDumpOnOutOfMemoryError -Djava.awt.headless=true -Xss512k -XX:SurvivorRatio=2 -XX:MaxTenuringThreshold=32 -XX:TargetSurvivorRatio=90"
  end
